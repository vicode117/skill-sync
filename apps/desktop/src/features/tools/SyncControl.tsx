    ...
    return (
      <div className="mt-3 border-t pt-3 text-xs" data-testid="sync-report">
        <p className={report.failed.length === 0 ? "text-success" : "text-warning"} role="status">
          {report.dryRun ? "Dry run — " : ""}Sync {toolName}: {reportSummary(report)}
        </p>
        {report.failed.map((f) => (
    ...
    return (
      <div className="mt-3 border-t pt-3">
        <Button variant="outline" size="sm" onClick={() => void preview()} disabled={busy}>
          ...
          {busy ? "Planning…" : "Sync from canonical store"}
        </Button>
        {error ? (
  ...
  return (
    <div className="mt-3 space-y-2 border-t pt-3 text-xs" data-testid="sync-plan">
      <p className="font-medium">
        Sync plan — {plan.method} into{" "}
        <span className="font-mono">{plan.targetDir ?? "<no location>"}</span>
      </p>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        >
          {busy ? "Syncing…" : `Apply (${mutations.length} change${mutations.length === 1 ? "" : "s"})`}
        </Button>
